...
### Modeling
```{r}
## Slitting data sets into training and test data. (80/20)
set.seed(100)

train_indices_log <- createDataPartition(semifinal_data_log_pred$accident_count, p = 0.8, list = FALSE)
train_data_log <- semifinal_data_log_pred[train_indices_log, ]
test_data_log <- semifinal_data_log_pred[-train_indices_log, ]

train_indices_cbrt <- createDataPartition(semifinal_data_cbrt_pred$accident_count, p = 0.8, list = FALSE)
train_data_cbrt <- semifinal_data_cbrt_pred[train_indices_cbrt, ]
test_data_cbrt <- semifinal_data_cbrt_pred[-train_indices_cbrt, ]
...
train_data_log_pred <- train_data_log[order(train_data_log$modzcta, train_data_log$year_month), ]
train_data_log_pred$time_index <- as.numeric(as.factor(train_data_log_pred$year_month))
...
## Also need to alter time column for index
semifinal_data_cbrt_pred <- train_data_cbrt[order(train_data_cbrt$modzcta, train_data_cbrt$year_month), ]
...
### Testing the CB RT Model Test Data

```{r}
# Predciting Accidents 
test_data_cbrt$predicted_accidents <- predict(model_zip_cb, newdata = test_data_cbrt, type = "response")


actual <- test_data_cbrt$accident_count
predicted <- test_data_cbrt$predicted_accidents
...
```{r}
ggplot(test_data_cbrt, aes(x = predicted, y = actual)) +
  geom_point(alpha = 0.4) +
  geom_abline(slope = 1, intercept = 0, color = "red") +
  labs(title = "Actual vs Predicted Accidents",
       x = "Predicted",
       y = "Actual") 
...
```{r}
test_summary_cbrt <- test_data_cbrt %>% group_by(modzcta) %>%  summarise(actual = sum(accident_count),predicted = sum(predicted_accidents))

ggplot(test_summary_cbrt, aes(x = actual, y = predicted)) +
  geom_point() +
  geom_abline(slope = 1, intercept = 0, color = "blue") +
  labs(title = "ZIP Code-level Actual vs Predicted Totals")

```

``` {r}
#Mapping data
prediction_map_df <- left_join(zcta, test_data_cbrt, by = "modzcta")

ggplot(prediction_map_df) +
  geom_sf(aes(fill = predicted_accidents), color = "white") +
  scale_fill_viridis_c(option = "plasma") +
  labs(title = "Predicted Traffic Accidents by ZCTA",
...
ggplot(prediction_map_df) +
  geom_sf(aes(fill = accident_count), color = "white") +
  scale_fill_viridis_c(option = "plasma") +
  labs(title = "Actual Accident Counts by ZIP",
       fill = "Actual") +
  theme_minimal()+
  theme(
    panel.grid = element_blank(),
...
```{r}
prediction_map_df <- prediction_map_df %>%  mutate(error = predicted_accidents - accident_count)

ggplot(prediction_map_df) +
  geom_sf(aes(fill = error), color = "white") +
  scale_fill_gradient2(low = "blue", mid = "white", high = "red", midpoint = 0) +
  labs(title = "Prediction Error by ZIP Code",
       fill = "Residual (Pred - Actual)")+
    theme_minimal()+
  theme(
    panel.grid = element_blank(),
    axis.text = element_blank(),
...
## Model under predicted in the ZCTA that has the most accidents generally speaking. 
```
