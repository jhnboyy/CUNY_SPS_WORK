...
We can get a better idea of the distributions and skewness by plotting our continuous variables: <br> \blandscape
...
train$HOME_VAL_CAT <- factor(train$HOME_VAL_CAT, levels = c("Very Low", "Low", "Medium", "High", "Missing"))
train$HOME_VAL_CAT[is.na(train$HOME_VAL)] <- "Missing"
...
eval$HOME_VAL_CAT <- factor(eval$HOME_VAL_CAT, levels = c("Very Low", "Low", "Medium", "High", "Missing"))
eval$HOME_VAL_CAT[is.na(eval$HOME_VAL)] <- "Missing"
...
impute_func <- function(data, data_type) {
    ...
    meth <- ini$meth
    meth['AGE'] <- 'mean'  # Example: Custom method for 'AGE'
...
calculate_transformations <- function(data) {
  list(
    ...
    OLDCLAIM_cs = function(x) scale(x),  # this will create a scaled version
...
apply_pre_calculated_transformations <- function(data, transforms) {
  data %>%
    mutate(
      ...
      OLDCLAIM_transformed = transforms$OLDCLAIM_cs(OLDCLAIM)[, 1],  # Apply and keep the transformed version
      OLDCLAIM = OLDCLAIM,  # Keep the original
...
columns_to_check <- c("AGE", "BLUEBOOK", "INCOME", "MVR_PTS", "OLDCLAIM", "TIF", "TRAVTIME", "YOJ", "CLM_FREQ", "CAR_AGE") #continuous 
...
encoded_columns <- c("SEX.F", "SEX.M", "EDUCATION.L", "EDUCATION.Q", "EDUCATION.C", 
                     ...
                     "HOME_VAL_CAT.Medium", "HOME_VAL_CAT.High", "HOME_VAL_CAT.Missing", "RED_CAR.No", 
...
stepwise_regression <- function(data, response_var) {
  ...
  predictors_for_stepwise <- c("`AGE`", "`BLUEBOOK`", "`INCOME`", 
                  ...
                  "`HOME_VAL_CAT.High`", "`HOME_VAL_CAT.Missing`", "`RED_CAR.No`", 
