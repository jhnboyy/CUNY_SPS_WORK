...
def exercise01():
    ...
    df = pd.DataFrame(np.array([[1,2,3,4],[5,2,3,4],[4,7,8,8]]))
...
def exercise08():
    ...
    # ------ Place code below here \/ \/ \/ ------
    resp = requests. get("https://tinyurl.com/y63q7okz")
    data = [i.split(',') for i in resp.content.decode("utf-8").splitlines()]
    df = pd.DataFrame(data[1:], columns = data[0])
    row_count = df.shape[0]
    avg_sq_ft = round(df["sq__ft"].astype(int).mean(),2)
...
def exercise11(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    array_1d = np.random.randint(1, n-1, size=(n))
...
def exercise12(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    checkerboard_matrix = (np.indices((n*2, n*2)).sum(axis=0)+1)%2
    # ------ Place code above here /\ /\ /\ ------ 
...
def exercise13(n):
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    count_days = pd.date_range(pd.Timestamp('01/01/2010').date(),pd.Timestamp.today().date())[:n]
    s = pd.Series(np.random.randint(0, n, size=(len(count_days))),index=[pd.to_datetime(i) for i in count_days])
    df =pd.DataFrame(s)
    plt.plot(df[0].cumsum())
...
def exercise14(words):
    ...
    # ------ Place code below here \/ \/ \/ ------
    s = pd.Series(words)
    len_s = s.map(lambda x: len(x))
    df = pd.DataFrame([s,len_s]).T
    #Not clear on whether we want the words in df too
    # ------ Place code above here /\ /\ /\ ------ 
    return df
...
def exercise15():
    ...
    data = [i.split(',') for i in resp.content.decode("utf-8").splitlines()]
    df = pd.DataFrame(data[1:], columns = data[0])
    df = df[['street','zip']].iloc[[i for i in range(df.shape[0]) if (i+1)%5==0]]
