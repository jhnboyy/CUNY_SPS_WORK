...
def exercise01():
    ...
    df1 = pd.DataFrame({"Name":["Jeff","Caleb","Aga","Emily","Gary"],"Age":[20,34,30,45,34]})
...
def exercise02():
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df2 = pd.read_csv("data.csv") #Sitting locally in relation to his file
    print(df2.head(n=5))
    # ------ Place code above here /\ /\ /\ ------
    return df2
...
def exercise04():
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    df4 = pd.DataFrame([{"city":"Rome","data":"Italy"},
                        {"city":"New York","data":"United States"},
                        {"city":"London","data":"United Kingdom"}])
    # ------ Place code above here /\ /\ /\ ------
    return df4
...
def exercise06():
    ...
    '''
    # ------ Place code below here \/ \/ \/ ------
    ## The file has id name and value as columns, The code breaks if i dont manually assign column names as described above.
    df6 = pd.read_excel("data.xlsx", header = None)
    df6.columns = ["A","B","C"]
#     df6 = pd.read_excel("data.xlsx",columns =["A","B","C"], headers=False)
    # ------ Place code above here /\ /\ /\ ------
    return df6
